...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        drop_last=True,
    )  # in training we drop the last incomplete minibatch
...
def validate(
    val_loader, model, act_criterion, comp_criterion, regression_criterion, iter
):
    batch_time = AverageMeter()
    losses = AverageMeter()
    act_losses = AverageMeter()
    ...
    comp_group_size = (
        val_loader.dataset.fg_per_video + val_loader.dataset.incomplete_per_video
    )
    for (
        i,
        (
    ...
    print(
        (
            ...
            "Act Accuracy {act_acc.avg:.02f} FG Acc. {fg_acc.avg:.02f} BG Acc. {bg_acc.avg:.02f}".format(
                act_loss=act_losses,
                comp_loss=comp_losses,
                loss=losses,
                act_acc=act_accuracies,

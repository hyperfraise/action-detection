...
# options
parser = argparse.ArgumentParser(description="Evaluate detection performance metrics")
...
nms_threshold = (
    ...
    else dataset_configs["evaluation"]["nms_threshold"]
)
top_k = args.top_k if args.top_k else dataset_configs["evaluation"]["top_k"]
softmax_bf = (
    args.softmax_before_filter
    if args.softmax_before_filter
    else dataset_configs["evaluation"]["softmax_before_filter"]
)
...
def gen_detection_results(video_id, score_tp):
    ...
    if top_k <= 0 and cls_score_dict is None:
        ...
        for i in range(num_class):
            ...
            try:
                dataset_detections[i][video_id] = np.concatenate(
                    (rel_prop, combined_scores[:, i][:, None], loc_scores, dur_scores),
                    axis=1,
                )
            except:
        ...
        video_cls_score = cls_score_dict[
            os.path.splitext(os.path.basename(video_id))[0]
        ]
        ...
        for video_cls in np.argsort(video_cls_score)[-args.cls_top_k :]:
            ...
            try:
                dataset_detections[video_cls][video_id] = np.concatenate(
                    (
                        rel_prop,
                        combined_scores[:, video_cls][:, None],
                        loc_scores,
                        dur_scores,
                    ),
                    axis=1,
...
for k, v in list(detection_scores.items()):
...
for cls in range(num_class):
    dataset_detections[cls] = {
        ...
        for k, v in list(dataset_detections[cls].items())
    }
print("NMS Done.")
...
if not args.no_regression:
    ...
    for cls in range(num_class):
        dataset_detections[cls] = {
            k: perform_regression(v) for k, v in list(dataset_detections[cls].items())
        }

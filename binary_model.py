...
class BinaryClassifier(torch.nn.Module):
    ...
    def _construct_diff_model(self, base_model, keep_rgb=False):
        ...
        if keep_rgb:
            ...
            new_kernels = torch.cat(
                (params[0].data.mean(dim=1).expand(new_kernel_size).contiguous()), 1
            )
            new_kernel_size = (
            ...
            new_kernels = (
                params[0].data.mean(dim=1).expand(new_kernel_size).contiguous()
            )
    ...
    def train_forward(self, inputdata, target):
        ...
        base_out = self.base_model(
            inputdata.view((-1, sample_len) + inputdata.size()[-2:])
        )
    ...
    def prepare_test_fc(self):
        ...
        self.test_fc = nn.Linear(
            self.classifier_fc.in_features, self.classifier_fc.out_features
        )

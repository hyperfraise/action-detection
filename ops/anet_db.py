...
class ANetDB(object):
    ...
    def get_db(cls, version="1.2"):
        ...
        raw_db_file = "data/activity_net.v{}.min.json".format(
            "-".join(version.split("."))
        )
    ...
    def prepare_data(self, raw_db):
        ...
        self._video_dict = {
            k: Video(k, v, self._name_idx_table) for k, v in self._database.items()
        }

        # split testing/training/validation set
        self._testing_dict = OrderedDict(
            sorted(
                [(k, v) for k, v in self._video_dict.items() if v.subset == "testing"],
                key=lambda x: x[0],
            )
        )
        self._training_dict = OrderedDict(
            sorted(
                [(k, v) for k, v in self._video_dict.items() if v.subset == "training"],
                key=lambda x: x[0],
            )
        )
        self._validation_dict = OrderedDict(
            sorted(
                [
                    (k, v)
                    for k, v in self._video_dict.items()
        ...
        self._training_inst_dict = {
            i.name: i for v in self._training_dict.values() for i in v.instances
        }
        self._validation_inst_dict = {
            i.name: i for v in self._validation_dict.values() for i in v.instances
        }

        print(
            "There are {} videos for training, {} for validation, {} for testing".format(
                len(self._training_dict),
                len(self._validation_dict),
                len(self._testing_dict),
            )
        )
        print(
            "There are {} instances for training, {} for validataion".format(
    ...
    def _parse_taxonomy(self):
        ...
        leaf_nodes = [
            name_dict[x] for x in list(set(name_dict.keys()).difference(parents))
        ]
        sorted_lead_nodes = sorted(leaf_nodes, key=lambda l: l["nodeName"])
        self._idx_name_table = {
            i: e["nodeName"] for i, e in enumerate(sorted_lead_nodes)
        }
        self._name_idx_table = {
            e["nodeName"]: i for i, e in enumerate(sorted_lead_nodes)
        }
        self._name_table = {x["nodeName"]: x for x in sorted_lead_nodes}
        print(
            "Got {} leaf classes out of {}".format(
                len(self._name_table), len(name_dict)
            )
        )

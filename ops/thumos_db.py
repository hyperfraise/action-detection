...
class Instance(object):
    ...
    def covering_ratio(self):
        return (
            self._starting / float(self._total_duration),
            self._ending / float(self._total_duration),
        )
    ...
    def path(self):
        if self._file_path is None:
            raise ValueError(
                "This instance is not associated to a file on disk. Maybe the file is missing?"
            )
...
class Video(object):
    ...
    def __init__(self, key, info, name_idx_mapping=None):
        ...
        self._instances = [
            Instance(i, x, self._id, self._info_dict, name_idx_mapping)
    ...
    def path(self):
        if self._file_path is None:
            raise ValueError(
                "This video is not associated to a file on disk. Maybe the file is missing?"
            )
        return self._file_path
...
class THUMOSDB(object):
    ...
    def __init__(self, token):
        ...
        if token is not self._CONSTRUCTOR_LOCK:
            raise ValueError(
                "Use get_db to construct an instance, do not directly use the constructor"
            )
    ...
    def prepare_data(self, db_folder):
        def load_subset_info(subset):
            ...
            annotation_files = glob.glob(
                os.path.join(db_folder, annotation_folder, "*")
            )
            avoid_file = "{}_avoid_videos.txt".format(subset)

            durations_lines = [
                x.strip() for x in open(os.path.join(db_folder, duration_file))
            ]
            annotaion_list = [
                (os.path.basename(f).split("_")[0], list(open(f)))
                for f in annotation_files
            ]
            avoid_list = [
                x.strip().split() for x in open(os.path.join(db_folder, avoid_file))
            ]
            ...
            # process video info
            video_names = [
                durations_lines[i].split(".")[0]
                for i in range(0, len(durations_lines), 2)
            ]
            video_durations = [
                durations_lines[i] for i in range(1, len(durations_lines), 2)
            ]
            ...
            for cls_name, annotations in annotaion_list:
                for a in annotations:
                    ...
                    if ("{}-{}".format(vid, cls_name) not in avoid_set) and (
                        st <= float(duration_dict[vid])
                    ):
        ...
        def construct_video_dict(video_info, annotaion_table, subset, name_idx_mapping):
            ...
            for v in video_info:
                info_dict = {
                    ...
                    "url": None,
                    "annotations": [
                        {"label": item[0], "segment": (item[1], item[2])}
                        for item in annotaion_table[v[0]]
                        if item[0] not in self.ignore_labels
                    ],
                }
        ...
        self._validation_dict, self._validation_inst_dict = construct_video_dict(
            self._validation_info[0],
            self._validation_info[1],
            "validation",
            self._name_idx_table,
        )
        self._test_dict, self._test_inst_dict = construct_video_dict(
            self._test_info[0], self._test_info[1], "test", self._name_idx_table
        )
        self._video_dict = dict(
            list(self._validation_dict.items()) + list(self._test_dict.items())
        )
    ...
    def _parse_taxonomy(self):
        ...
        validation_names = sorted(
            [os.path.split(x)[1].split("_")[0] for x in self._validation_info[-1]]
        )
        test_names = sorted(
            [os.path.split(x)[1].split("_")[0] for x in self._test_info[-1]]
        )

        if len(validation_names) != len(test_names):
            raise IOError(
                "Validation set and test have different number of classes: {} v.s. {}".format(
        ...
        for i in range(len(validation_names)):
            if validation_names[i] != test_names[i]:
                raise IOError(
                    "Validation set and test have different class names: {} v.s. {}".format(
        ...
        print(
            "Got {} classes for the year {}".format(
                len(self._idx_name_table), self.year
            )
        )

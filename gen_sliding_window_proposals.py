...
parser.add_argument(
    "--dataset", default="activitynet", choices=["thumos14", "activitynet"]
)
...
proposal_list = list(
    [
        gen_exponential_sw_proposal(
            x, overlap=args.overlap, time_step=args.time_step, max_level=args.max_level
        )
        for x in videos
    ]
)
print(
    (
        "average # of proposals: {} at overlap param {}".format(
...
for th in IOU_thresh:
    ...
    recall_list.append(
        [args.overlap, th, np.mean(list(map(len, proposal_list))), pv, pi]
    )
print(
    (
        "average per video recall: {:.2f}, average per instance recall: {:.2f}".format(
...
dumped_list = [
    ...
    for v, prs in zip(videos, named_proposal_list)
    if v.id not in avoid_list
]

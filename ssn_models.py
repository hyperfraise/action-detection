...
class SSN(torch.nn.Module):
    ...
    SSN Configurations:
        ...
        stpp_configs:       {} 
            """.format(
                    ...
                    self.new_length,
                    self.dropout,
                    "ON" if self.with_regression else "OFF",
                    self.bn_mode,
                    stpp_cfg,
                )
            )
    ...
    def _prepare_ssn(self, num_class, stpp_cfg):
        ...
        self.stpp = StructuredTemporalPyramidPooling(
            feature_dim, True, configs=stpp_cfg
        )
        ...
        if self.with_regression:
            self.regressor_fc = nn.Linear(
                self.stpp.completeness_feat_dim(), 2 * num_class
            )
    ...
    def prepare_test_fc(self):
        ...
        reorg_comp_weight = (
            self.completeness_fc.weight.data.view(
                ...
                self.stpp.feat_multiplier,
                self.activity_fc.in_features,
            )
            .transpose(0, 1)
            .contiguous()
            .view(-1, self.activity_fc.in_features)
        )
        reorg_comp_bias = (
            self.completeness_fc.bias.data.view(1, -1)
            .expand(self.stpp.feat_multiplier, self.completeness_fc.out_features)
            .contiguous()
            .view(-1)
            / self.stpp.feat_multiplier
        )
        ...
        if self.with_regression:
            ...
            reorg_reg_bias = (
                ...
                .expand(self.stpp.feat_multiplier, self.regressor_fc.out_features)
                .contiguous()
                .view(-1)
                / self.stpp.feat_multiplier
            )
    ...
    def train_forward(self, input, aug_scaling, target, reg_target, prop_type):
        ...
        activity_ft, completeness_ft = self.stpp(
            base_out,
            aug_scaling,
            [
                self.starting_segment,
        ...
        if self.with_regression:
            ...
            raw_regress_fc = self.regressor_fc(completeness_ft).view(
                -1, self.completeness_fc.out_features, 2
            )
            return (
                raw_act_fc[act_indexer, :],
                target[act_indexer],
                raw_comp_fc[comp_indexer, :],
                target[comp_indexer],
                raw_regress_fc[reg_indexer, :, :],
                target[reg_indexer],
                reg_target[reg_indexer, :],
            )
        else:
            return (
                raw_act_fc[act_indexer, :],
    ...
    def _construct_flow_model(self, base_model):
        ...
        first_conv_idx = list(
            [x for x in list(range(len(modules))) if isinstance(modules[x], nn.Conv2d)]
        )[0]

...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            loss = validate(
                val_loader, model, binary_criterion, (epoch + 1) * len(train_loader)
            )
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (out_frames, out_prop_type) in enumerate(train_loader):
        ...
        fg_acc = accuracy(
            binary_score.view(-1, 2, binary_score.size(1))[:, 0, :].contiguous(),
            prop_type_target.view(-1, 2)[:, 0].contiguous(),
        )
        bg_acc = accuracy(
            binary_score.view(-1, 2, binary_score.size(1))[:, 1, :].contiguous(),
        ...
        if i % args.print_freq == 0:
            print(
                (
                    ...
                    "\n FG{fg_acc.val:.02f}({fg_acc.avg:.02f}) BG {bg_acc.val:.02f} ({bg_acc.avg:.02f})".format(
                        epoch,
                        i,
                        len(train_loader),
...
def validate(val_loader, model, criterion, iter):
    ...
    print(
        (
            "Testing Results: Loss {loss.avg:.5f} \t"
            "FG Acc. {fg_acc.avg:.02f} BG Acc. {bg_acc.avg:.02f}".format(
                loss=losses, fg_acc=fg_accuracies, bg_acc=bg_accuracies
            )
        )
    )
...
def save_checkpoint(state, is_best, filename="checkpoint.pth.tar"):
    filename = "binaryclassifier" + "_".join(
        (args.snapshot_pref, args.dataset, args.arch, args.modality.lower(), filename)
    )

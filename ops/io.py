...
def process_proposal_list(norm_proposal_list, out_list_name, frame_dict):
    ...
    for idx, prop in enumerate(norm_proposals):
        ...
        gt = [
            [int(x[0]), int(float(x[1]) * frame_cnt), int(float(x[2]) * frame_cnt)]
            for x in prop[2]
        ]
        ...
        prop = [
            [
                ...
                float(x[1]),
                float(x[2]),
                int(float(x[3]) * frame_cnt),
                int(float(x[4]) * frame_cnt),
            ]
            for x in prop[3]
        ]
        ...
        prop_dump = "\n".join(
            ["{} {:.04f} {:.04f} {:d} {:d}".format(*x) for x in prop]
        ) + ("\n" if len(prop) else "")
...
def dump_window_list(
    video_info, named_proposals, frame_path, name_pattern, allow_empty=False, score=None
):
    ...
    gt_windows = [
        (x[0] + 1, int(x[1][0] * real_fps), int(x[1][1] * real_fps)) for x in gt_w
    ]
    ...
    for pr in named_proposals:
        ...
        overlap = pr[1]
        overlap_self = pr[2]
        dump_proposals.append(
            "{} {:.04f} {:.04f} {} {}".format(
                label, overlap, overlap_self, real_start, real_end
            )
        )

    ret_str = "{path}\n{duration}\n{fps}\n{num_gt}\n{gts}{num_window}\n{prs}\n".format(
        path=os.path.join(frame_path, video_name),
        duration=frame_cnt,
        fps=1,
        num_gt=len(dump_gt),
        gts="\n".join(dump_gt) + ("\n" if len(dump_gt) else ""),
        num_window=len(dump_proposals),
        prs="\n".join(dump_proposals),
    )

...
parser = argparse.ArgumentParser(description="SSN Testing Tool")
...
gpu_list = args.gpus if args.gpus is not None else list(range(8))
...
def runner_func(dataset, state_dict, stats, gpu_id, index_queue, result_queue):
    ...
    reorg_stpp = STPPReorgainzed(
        output_dim, num_class + 1, num_class, num_class * 2, True, stpp_cfg=stpp_configs
    )
    ...
    while True:
        ...
        for frames in frames_gen:
            input_var = torch.autograd.Variable(
                frames.view(-1, length, frames.size(-2), frames.size(-1)).cuda(),
                volatile=True,
            )
            ...
            cnt += sc.size(0)
        act_scores, comp_scores, reg_scores = reorg_stpp.forward(
        ...
        result_queue.put(
            (
                dataset.video_list[index].id,
                rel_props.numpy(),
                act_scores.cpu().numpy(),
                comp_scores.cpu().numpy(),
                reg_scores.cpu().numpy(),
                output.cpu().numpy(),
            )
        )
...
if __name__ == "__main__":
    ...
    if not args.use_reference and not args.use_kinetics_reference:
        ...
        model_url = get_reference_model_url(
            args.dataset,
            args.modality,
            "ImageNet" if args.use_reference else "Kinetics",
            args.arch,
        )
    ...
    print(
        ("model epoch {} loss: {}".format(checkpoint["epoch"], checkpoint["best_loss"]))
    )
    base_dict = {
    ...
    for i in range(max_num):
        ...
        print(
            (
                "video {} done, total {}/{}, average {:.04f} sec/video".format(
                    i, i + 1, max_num, float(cnt_time) / (i + 1)
                )
            )
        )

    if args.save_scores is not None:
        save_dict = {k: v[:-1] for k, v in list(out_dict.items())}
        import pickle
    ...
    if args.save_raw_scores is not None:
        ...
        pickle.dump(
            save_dict, open(args.save_raw_scores, "wb"), pickle.HIGHEST_PROTOCOL
        )
